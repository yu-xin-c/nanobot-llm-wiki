...
> **为什么做这个：** NanoBot 的[官方记忆设计](https://github.com/HKUDS/nanobot/blob/main/docs/memory.md)已经具备分层存储、Dream 整理和 Git 版本回溯，但原生记忆仍主要由 `history.jsonl` 与 Markdown 文件承载，目前还没有面向用户的主题化 Wiki、关系图谱和可视化管理界面。与此同时，OpenClaw 已将 [`memory-wiki`](https://docs.openclaw.ai/plugins/memory-wiki) 作为内置插件，在基础记忆之外增加可维护的结构化 Wiki 知识层，为这条产品方向提供了明确的开源先例。

> **一句话介绍：** NanoBot LLM Wiki 把这套思路带到 NanoBot：它将用户偏好、项目背景、历史决策和导入文档保存为本地 Markdown 页面，并通过 SQLite 全文检索、关系图谱和可视化界面，让人和 Agent 都能搜索、核查、编辑并持续维护长期记忆。
...
- **Agent 原生**：通过 NanoBot Python entry points 注册 9 个 Wiki 工具，无需修改 NanoBot 核心代码。
...
唯一的安装前提是机器上已有 [`uv`](https://docs.astral.sh/uv/)。模型 API key
只在启动 NanoBot 前需要，不影响插件安装。

```bash
curl -fsSL https://raw.githubusercontent.com/yu-xin-c/nanobot-llm-wiki/main/scripts/install.sh | bash
```

安装器会在缺少 NanoBot 时自动安装它，将插件附加到 uv 管理的 NanoBot Python 环境，
暴露 `nanobot-wiki` 命令，初始化工作区，并自动运行 `doctor`。已有的 uv NanoBot、其他
插件和用户 Wiki 数据不会被重置；重复执行同一命令即可安全更新。

如果安装器提示 uv 命令目录不在 `PATH`，执行一次：

```bash
uv tool update-shell
```

首次使用 NanoBot 时，先完成模型配置；已有可用配置可以跳过：
...
安装器不会创建模型 API key，不会覆盖已有 `config.json`，也不会改写用户维护的
`MEMORY.md` 内容、Wiki 页面或自定义 skill。它只更新自己带标记的记忆桥和生成型 skill。

### 升级

重新执行快速开始中的安装命令即可。安装器只更新 NanoBot 环境中的 Wiki 插件并再次运行诊断，
不会清空工作区或移除其他插件。

如果先执行了 `uv tool upgrade nanobot-ai`，请随后再运行一次本项目安装器。`uv` 升级 NanoBot
时可能重建它的隔离环境，安装器会无损地重新附加 Wiki 插件。
...
卸载器只移除生成的记忆桥、生成型 skill、插件包和 `nanobot-wiki` 命令；NanoBot、其他插件和
`memory/wiki/` 中的数据都会保留。以后重新运行安装器即可接回原有 Wiki。指定了自定义工作区时，
卸载也应传入相同的 `NANOBOT_WORKSPACE`。

确认不再需要数据后，可自行删除保留的目录：
...
关系会持久化到可迁移的 `links.jsonl`，同时进入 SQLite 图索引并显示在图谱界面中。
省略 `unlink` 的 `--relation` 会移除这两个页面之间的全部定向关系。
...
```mermaid
flowchart LR
  User["用户或 NanoBot"] --> Tools["9 个 Wiki 工具"]
  Tools --> Store["WikiStore"]
  Store --> Markdown["Markdown 页面"]
  Store --> LinkSource["links.jsonl 关系源"]
...
2. 相关图谱关系和全文索引从活跃 SQLite 索引中移除。
3. 默认将 Markdown 移入 `archive/`，并保留可迁移的关系记录，便于审计和手工恢复。
4. 使用 CLI `--delete` 或工具参数 `archive=false` 才会永久删除页面文件及其关系源记录。
...
| `wiki_unlink(from_selector, to_selector, relation)` | 移除指定关系；省略关系类型时移除全部定向关系。 |
...
- 可用于共享演示的只读模式，以及 `/wiki/` 等反向代理子路径。

默认只监听 `127.0.0.1`。公开演示请启用 `--read-only`；需要在线编辑时，应通过 HTTPS 和额外访问控制反向代理，切勿把管理界面直接绑定到公网地址。
...
~/.nanobot/workspace/
  memory/
    ...
    wiki/
      wiki.db                  # 可重建的页面、全文检索和图索引
      links.jsonl              # 可迁移、可版本控制的页面关系源
...
Markdown 页面和 `links.jsonl` 是便于检查、迁移和版本控制的原始记录，SQLite 是可重建的
页面、检索与图索引层。备份整个
...
- FTS 行数是否漂移。
- `links.jsonl` 是否有效并与活跃图索引一致。
- `MEMORY.md` 记忆桥。
- `llm-wiki` skill。
- 工作区配置文件。
- 9 个 NanoBot 工具 entry points。
...
- Wiki 页面、关系源和 SQLite 数据库默认保存在本机 NanoBot 工作区。
- 插件本身不增加独立的云数据库或遥测服务。
...
- Markdown + JSONL 持久化，以及可重建的 SQLite 索引。
- 精确匹配、全文检索和子串兜底。
- 9 个 NanoBot 工具。
- 页面管理、知识库导入、图谱和诊断 UI。
- 手工 Markdown 重建索引。
- CLI、工具注册、HTTP API 和真实 NanoBot 隔离安装测试。
...
CI 会在 push 和 pull request 时运行单元测试、Ruff、NanoBot 安装冒烟测试和包构建。
...
NanoBot 会从它实际运行的 Python 环境中发现工具。重新运行安装器会把插件附加到该环境，
同时保留其中已有的其他扩展。
